...
public open class DerivativeStructureRing<T, A>(
    ...
    NumbersAddOperations<DerivativeStructure<T, A>> where A : Ring<T>, A : NumericAlgebra<T>, A : ScaleOperations<T> {
    ...
    override fun add(a: DerivativeStructure<T, A>, b: DerivativeStructure<T, A>): DerivativeStructure<T, A> {
        a.compiler.checkCompatibility(b.compiler)
        val ds = DerivativeStructure(a)
        a.compiler.add(a.data, 0, b.data, 0, ds.data, 0)
        return ds
    }
    ...
    override fun multiply(a: DerivativeStructure<T, A>, b: DerivativeStructure<T, A>): DerivativeStructure<T, A> {
        a.compiler.checkCompatibility(b.compiler)
        val result = DerivativeStructure(this, a.compiler)
        a.compiler.multiply(a.data, 0, b.data, 0, result.data, 0)
        return result
    }

    override fun DerivativeStructure<T, A>.minus(b: DerivativeStructure<T, A>): DerivativeStructure<T, A> {
        compiler.checkCompatibility(b.compiler)
        val ds = DerivativeStructure(this)
        compiler.subtract(data, 0, b.data, 0, ds.data, 0)
        return ds
    }

    override operator fun DerivativeStructure<T, A>.plus(b: Number): DerivativeStructure<T, A> {
        val ds = DerivativeStructure(this)
        ds.data[0] = algebra { ds.data[0] + number(b) }
        return ds
    }

    override operator fun DerivativeStructure<T, A>.minus(b: Number): DerivativeStructure<T, A> = this + -b.toDouble()
    override operator fun Number.plus(b: DerivativeStructure<T, A>): DerivativeStructure<T, A> = b + this
    override operator fun Number.minus(b: DerivativeStructure<T, A>): DerivativeStructure<T, A> = b - this
}
...
public class DerivativeStructureField<T, A : ExtendedField<T>>(
    ...
    override fun divide(a: DerivativeStructure<T, A>, b: DerivativeStructure<T, A>): DerivativeStructure<T, A> {
        a.compiler.checkCompatibility(b.compiler)
        val result = DerivativeStructure(this, a.compiler)
        a.compiler.divide(a.data, 0, b.data, 0, result.data, 0)
        return result
    }
